...
import sys
from PyQt5.QtWidgets import QApplication, QTextBrowser, QComboBox, QMainWindow, QAction, qApp, QMessageBox, QLineEdit, \
...
class Window2(QMainWindow):
    def __init__(self):
        ...
        #lb1 = QLabel(self)      #self를 어디에쓰냐에따라 오류 나고 안나고 함;;; lb1 = self.QLabel이 왜 안될까?
        # btn1.show() 안써도 나타남... 언제쓰고 언제 안쓰는건지..(아래 작성함)
        ...
        self.editor = QTextEdit(self)   #editor앞에 self를 안붙이면 클래스 내 다른 함수에서 호출 불가!
        self.editor.move(10, 60)
...
class Window(QMainWindow):
    def __init__(self):
        ...
        # 메뉴명 선택을 위한 라인 에디터(08/24추가)
        # cb.activated이벤트 발생 시 콤보박스 요소들도 변경되도록 처리하여 선택하게 할 수도 있지만
        # 사용자 입력을 구현하기 위해 다음을 작성함.
        self.qle = QLineEdit(self) #여기서 self.qle = QLineEdit(), self.qle.show()만 작성하면 새 창이 뜬다!
        self.qle.setStatusTip("레시피명을 입력 후 Enter (Ctrl+C, Ctrl+V를 사용하시면 편합니다)")
        self.qle.move(50, 280)
        self.qle.returnPressed.connect(self.return_menu)

        self.message = QLabel(self)
        self.message.setText("<< 메뉴명 입력(레시피 보기)")
        self.message.move(150, 290)
